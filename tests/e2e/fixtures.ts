...
const mysql = `mysql -h ${dbHost} -u ${dbUser} ${dbName} --ssl-mode=DISABLED --default-character-set=utf8mb4`;
const snapshotFile = '/tmp/openvk-test-db-snapshot.sql';

export const test = base.extend({
  page: async ({ page }, use) => {
    execSync(`${mysql} < ${snapshotFile}`, { stdio: 'inherit', env: { ...process.env, MYSQL_PWD: dbPass } });
    await use(page);
  },
});
